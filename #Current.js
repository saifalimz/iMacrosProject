...
{
	var macro = "CODE:";
	macro += "'EMAIL EXTRACTION SCRIPT - SAIF ALI" + "\n";
	macro += "" + "\n";
...
for (var j = 1; j < 2; j++)
    {
        var macro2 = "CODE";
        macro2 += "URL GOTO=http://www.distributorcentral.com/supplier/maintenance.cfm" + "\n";
	macro2 += "TAG POS=1 TYPE=SELECT FORM=ID:custSearch ATTR=NAME:as_field CONTENT=%st" + "\n";
	macro2 += "TAG POS=1 TYPE=INPUT:TEXT FORM=ID:custSearch ATTR=ID:as_keyword CONTENT=New<SP>York" + "\n";
	macro2 += "TAG POS=1 TYPE=BUTTON FORM=ID:custSearch ATTR=TYPE:submit&&CLASS:fv-hidden-submit&&TXT:" + "\n";
	macro2 += "WAIT SECONDS=3" + "\n";
	macro2 += "TAG POS=1 TYPE=SELECT FORM=ACTION:/supplier/maintenance.cfm?as_sort_by=a.companyname&as_field=st&as_keyword=New%20York&asd=15&as_distact=&as_supact=&haswebsites=&as_acctstatuscode=&as_acct_rep=&epath=&as_starts_with= ATTR=NAME:as_display CONTENT=%150" + "\n";
	macro2 += "WAIT SECONDS=2" + "\n";
	macro2 += "TAG POS={{j}} TYPE=A ATTR=TXT:Next";
        iimPlay(macro2);
    }        